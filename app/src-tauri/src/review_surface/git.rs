...
///
/// `paths`, when non-empty, restricts the diff to those files — what
/// `file_impl` passes (a single path) instead of computing the whole
/// scope and searching it, and what a `ScopeFiles` snapshot (#171
/// slice (f)) passes for a whole batch, so an agent-API call that needs
/// several files' diffs still costs one diff rather than one per file.
/// `scope_impl` passes `&[]`, for the file list.
pub(super) fn scope_diffs(
    ...
    match scope {
        ...
        // hiding work.
        Scope::Branch => repo
            .diff_tree_to_workdir(range.base_sha.as_deref(), paths)
            .map_err(|e| e.to_string()),
        Scope::Commit => match commit_sha {
            Some(sha) => repo.diff_commit(sha, paths).map_err(|e| e.to_string()),
            None => Ok(Vec::new()),
        },
